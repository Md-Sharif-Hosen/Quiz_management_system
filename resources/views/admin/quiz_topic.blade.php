            ...
            <div class="card-body">
                ...
                <div class="card row col-md-3 d-inline-block" style="height:150px;width:300px; margin:10px;background-color: #eac5c5;  ">
                   ...
                   <h5 class="text-center">total Examiner: @if ($data->quiz_submit_user)
                    {{ $data->quiz_submit_user->where("quiz_id",$data->id)->pluck('user_id')->unique('user_id')->count()}}<a href="{{ route('quiz_examiner',$data->id) }}" class="btn btn-outline-info">view</a></h5> @else 0 <a href="{{ route('quiz_examiner',$data->id) }}" class="btn btn-outline-info">view</a></h4>
                   @endif
